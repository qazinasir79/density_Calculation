...
from eos_models import density_PR, density_SRK
import warnings
warnings.filterwarnings('ignore')

st.set_page_config(page_title="Ethane Density ML Dashboard", layout="wide")
st.title("Ethane Density Prediction: ML Models vs Experimental Data")

st.markdown("""
<div style="background:linear-gradient(135deg,#667eea,#764ba2);padding:18px 24px;border-radius:12px;margin-bottom:20px">
  <span style="color:white;font-size:20px;font-weight:700">🏆 Final Verdict: CatBoost is the Best Model</span>
  <span style="color:#ddd;font-size:14px;margin-left:16px">
    R²=0.999 · CV R²=0.888 · MAE=4.05 kg/m³ · MAPE=2.95% · Normal Residuals ✓
  </span>
</div>
...
def load_model(name):
    ...
    if name == 'Neural Network':
        model, scaler_X, scaler_y = joblib.load(f'models/{name}.pkl')
        return (model, scaler_X, scaler_y, 'nn')
    return (joblib.load(f'models/{name}.pkl'), None, None, 'sklearn')

@st.cache_data
def predict(name, X_input):
    obj = load_model(name)
    if obj[3] == 'eos':
        T = X_input[:, 0]
        P = X_input[:, 1]
        if name == 'Peng-Robinson EOS':
            return np.array([density_PR(t, p) for t, p in zip(T, P)])
        else:
            return np.array([density_SRK(t, p) for t, p in zip(T, P)])
    if obj[3] == 'nn':
        model, sx, sy, _ = obj
...
def metrics(y_t, y_p):
    return (r2_score(y_t, y_p), mean_absolute_error(y_t, y_p),
            np.sqrt(mean_squared_error(y_t, y_p)),
            np.mean(np.abs((y_t - y_p) / y_t)) * 100,
            np.max(np.abs(y_t - y_p)), np.mean(y_p - y_t))
...
show_refprop = st.sidebar.checkbox("Show RefProp 8.0 comparison", value=True)
show_virial = st.sidebar.checkbox("Show Virial Coefficients", value=True)

st.sidebar.markdown("---")
st.sidebar.subheader("Quick Predict")
qT = st.sidebar.number_input("Temperature (K)", min_value=298.0, max_value=450.0, value=350.0, step=1.0)
qP = st.sidebar.number_input("Pressure (MPa)", min_value=0.8, max_value=200.0, value=50.0, step=0.5)
X_q = np.array([[qT, qP]])
if st.sidebar.button("Predict", type="primary", use_container_width=True):
    y_q = predict(sel, X_q)[0]
    st.sidebar.success(f"**{sel}**\n\nρ = **{y_q:.4f}** kg/m³")

tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["Data & Predictions", "Statistical Metrics", "Model Comparison", "Virial Coefficients", "Custom Prediction"]
)

y_pred_sel = predict(sel, X)
...
with tab2:
    ...
    lo, hi = min(y_true.min(), y_pred_sel.min()), max(y_true.max(), y_pred_sel.max())
    fig = go.Figure()
    ...
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=res, nbinsx=30, name='Residuals'))
    mu, sg = np.mean(res), np.std(res)
    xn = np.linspace(res.min(), res.max(), 200)
    fig.add_trace(go.Scatter(x=xn, y=stats.norm.pdf(xn, mu, sg) * len(res) * (res.max()-res.min())/30,
    ...
    if show_refprop:
        st.subheader("Comparison with RefProp 8.0")
        ...
        comp = pd.DataFrame({
            ...
            'RefProp 8.0': [r2_r, mae_r, rmse_r, mape_r, maxe_r, bias_r]
        })
...
if show_all:
    with tab3:
        ...
        for eos_name in EOS_MODELS:
            yp = predict(eos_name, X)
            r, mae, rmse, mape, me, bias = metrics(y_true, yp)
            eos_rows.append({'Model': eos_name, 'R2_train': r, 'MAE_train': mae,
                             'RMSE_train': rmse, 'MAPE_train': mape})
        eos_df = pd.DataFrame(eos_rows)
        perf_all = pd.concat([perf, eos_df], ignore_index=True).sort_values('R2_train', ascending=False)
...
with tab4:
    if show_virial:
        st.subheader("Second & Third Virial Coefficients")
        vdf = pd.DataFrame({
        ...
        st.subheader("B(T) Correlation")
        T_inv = 1.0 / vdf['T (K)'].values
        coeffs = np.polyfit(T_inv, vdf['B (cm³/mol)'].values, 2)
        Tr = np.linspace(340, 460, 100)
        Bf = coeffs[0]/Tr**2 + coeffs[1]/Tr + coeffs[2]
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=vdf['T (K)'], y=vdf['B (cm³/mol)'],
            mode='markers', marker=dict(size=12, color='red'), name='Data'))
        fig.add_trace(go.Scatter(x=Tr, y=Bf, mode='lines',
            name='Quadratic in 1/T', line=dict(dash='dot')))
        fig.update_layout(title="B(T) Fit", xaxis_title="T (K)", yaxis_title="B (cm³/mol)", height=400)
        st.plotly_chart(fig, width='stretch')

with tab5:
    st.subheader("Custom Prediction")
    st.markdown("Enter temperature and pressure to predict ethane density.")

    col_i1, col_i2 = st.columns(2)
    with col_i1:
        cT = st.number_input("Temperature (K)", min_value=200.0, max_value=500.0,
                             value=350.0, step=1.0, key="custom_T")
    with col_i2:
        cP = st.number_input("Pressure (MPa)", min_value=0.5, max_value=250.0,
                             value=50.0, step=0.5, key="custom_P")

    X_custom = np.array([[cT, cP]])
    c_pred_sel = predict(sel, X_custom)[0]

    col_r1, col_r2, col_r3 = st.columns(3)
    col_r1.metric("Model", sel)
    col_r2.metric("Input Conditions", f"T = {cT:.1f} K, P = {cP:.2f} MPa")
    col_r3.metric("Predicted Density", f"{c_pred_sel:.4f} kg/m³")

    st.markdown("### Density vs Pressure at Selected Temperature")
    T_fixed = cT
    P_range = np.linspace(0.8, 200, 200)
    X_range = np.column_stack([np.full_like(P_range, T_fixed), P_range])
    y_range = predict(sel, X_range)

    fig_custom = go.Figure()
    fig_custom.add_trace(go.Scatter(x=P_range, y=y_range, mode='lines',
        name=f'{sel} at {T_fixed:.1f} K', line=dict(width=3)))
    exp_mask = np.abs(df['T_K'] - T_fixed) < 1
    if exp_mask.any():
    ...
    for m in MODEL_FILES:
        all_preds[m] = [predict(m, X_custom)[0]]
...
st.sidebar.markdown("### Features: T (298-450 K), P (0.8-200 MPa)")
